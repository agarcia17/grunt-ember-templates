...
function manualCompile(templateCompilerPath, template, grunt) {
  ...
  try {
    ...
    grunt.fail.warn(
      'ember-template-compiler.js is not found. You can install with `bower install ember --save`. If you would like to use a custom location you can setup `templateCompilerPath` option in Gruntfile.js. It has to point to ember-template-compiler.js which bundled with Ember.js. The default path: bower_components/ember/ember-template-compiler.js'
    );
  }
  ...
  vm.runInContext(
    'compiledJS = ((typeof Ember !== "undefined" && Ember.HTMLBars) || module.exports || exports).precompile(template);',
    context
  );
    ...
    this.files.forEach(function(f) {
        ...
        try {
          ...
          if (options.precompile) {
            ...
            contents =
              ...
              '.compile(' +
              JSON.stringify(grunt.file.read(file)) +
              ')';
          }
      ...
      if (options.concatenate) {
        ...
        processedTemplates.forEach(function(template) {
          writeFile(
            output.concat(template.contents),
